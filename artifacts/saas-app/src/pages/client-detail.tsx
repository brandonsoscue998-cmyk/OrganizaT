              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="rounded-lg bg-muted/50 px-3 py-2.5">
                    <p className="text-[11px] text-muted-foreground uppercase tracking-wide font-medium">Este mes</p>
                    <p className="text-2xl font-bold mt-0.5">{thisMonthSessions.length}</p>
                    <p className="text-[11px] text-muted-foreground">sesión{thisMonthSessions.length !== 1 ? "es" : ""}</p>
                  </div>
                  <div className="rounded-lg bg-muted/50 px-3 py-2.5">
                    <p className="text-[11px] text-muted-foreground uppercase tracking-wide font-medium">Total generado</p>
                    <p className="text-2xl font-bold mt-0.5">{formatCurrency(totalAmount)}</p>
                    <p className="text-[11px] text-muted-foreground">{sessions?.length ?? 0} sesiones</p>
                  </div>
                  <div className="rounded-lg bg-green-50 border border-green-100 px-3 py-2.5">
                    <p className="text-[11px] text-green-700 uppercase tracking-wide font-medium">Pagado</p>
                    <p className="text-2xl font-bold text-green-700 mt-0.5">{formatCurrency(paidAmount)}</p>
                    <p className="text-[11px] text-green-600">{(sessions ?? []).filter(s => s.paid).length} sesiones</p>
                  </div>
                  <div className={`rounded-lg px-3 py-2.5 ${pendingAmount > 0 ? "bg-yellow-50 border border-yellow-100" : "bg-muted/50"}`}>
                    <p className={`text-[11px] uppercase tracking-wide font-medium ${pendingAmount > 0 ? "text-yellow-700" : "text-muted-foreground"}`}>Pendiente</p>
                    <p className={`text-2xl font-bold mt-0.5 ${pendingAmount > 0 ? "text-yellow-700" : ""}`}>{formatCurrency(pendingAmount)}</p>
                    <p className={`text-[11px] ${pendingAmount > 0 ? "text-yellow-600" : "text-muted-foreground"}`}>{(sessions ?? []).filter(s => !s.paid).length} sesiones</p>
                  </div>
                </div>
                {pendingAmount > 0 && (
